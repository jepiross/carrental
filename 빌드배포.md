...
#### aws repository 로그인   
aws ecr get-login-password --region [리전] | docker login --username AWS --password-stdin [내고유번호-생성결과에서보임].dkr.ecr.[리전].amazonaws.com   
...
## gateway deploy
kubectl create deploy gateway --image=[내고유번호-생성결과에서보임].dkr.ecr.[리전].amazonaws.com/[ecr경로]-gateway:v1   
