...
이때, 재기동 제어값이 /tmp/healthy파일을 강제로 지워 liveness가 pod를 비정상 상태라고 판단하도록 하였다.    
...
# configmap
rental 서비스의 경우, 국가와 지역에 따라 설정이 변할 수도 있음을 가정하고, configmap에 설정된 국가와 지역 설정을 서비스 CONTAINER에서 받을 수 있도록 한다.   
```
$ kubectl apply -f - <<EOF
apiVersion: v1
kind: ConfigMap
...
EOF
```
rental deployment에서 rental-region(cm)의 값을 사용 할 수 있도록 수정한다.
``` yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: rental
          image: 496278789073.dkr.ecr.ap-northeast-2.amazonaws.com/ecr-skcc-team2-rental:v1
      ...
      - name: config
        configMap:
          name: rental-region
```
rental pod에 cm에서 환경변수를 가져오겠다는 설정을 확인 할 수 있다.
![](images/cm-1.PNG)

실제 rental pod안에서 cm에 설정된 국가와 지역 설정을 확인 할 수 있다.
![](images/cm-2.PNG)
